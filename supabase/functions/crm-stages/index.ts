...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (method === "GET") {
      const { data, error } = await supabase.from("lead_stages").select("*").order("position", { ascending: true });
    ...
    if (method === "POST") {
      ...
      if (!body.id || !body.name || !body.color || body.position === undefined) {
        return new Response(JSON.stringify({ error: "Missing required fields: id, name, color, position" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      const { data, error } = await supabase.from("lead_stages").insert({
        id: body.id, name: body.name, color: body.color, position: body.position,
        is_active: body.is_active ?? true, created_by: auth.userId,
      }).select().single();
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (method === "PATCH") {
      ...
      for (const f of ["name", "color", "position", "is_active"]) { if (body[f] !== undefined) updates[f] = body[f]; }
