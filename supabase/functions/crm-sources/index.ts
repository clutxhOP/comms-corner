...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (method === "GET") {
      const { data, error } = await supabase.from("lead_sources").select("*").order("position", { ascending: true });
    ...
    if (method === "POST") {
      ...
      if (!body.id || !body.name) {
        return new Response(JSON.stringify({ error: "Missing required fields: id, name" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      const { data, error } = await supabase.from("lead_sources").insert({
        id: body.id, name: body.name, position: body.position ?? 0,
        icon: body.icon || null, is_active: body.is_active ?? true,
      }).select().single();
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (method === "PATCH") {
      ...
      for (const f of ["name", "icon", "position", "is_active"]) { if (body[f] !== undefined) updates[f] = body[f]; }
